...
from src.libs.utils import load_yaml, read_spec, write_yaml
...
spec = read_spec(
    "../spec.txt"
)  # , other_params['n_pixel_h'], other_params['n_pixel_v'])
...
exp_path = (
    exp_root + "/" + spec["input_type"] + "_" + env_params["controller_type"] + "_" + ID
)
...
policy_net = DQN(
    n_observed_state, learning_params["layers"], env_params["action_space_size"]
).to(device)
target_net = DQN(
    n_observed_state, learning_params["layers"], env_params["action_space_size"]
).to(device)
...
for e in range(learning_params["n_episodes"]):
    ...
    while not crash and logger.tstep < learning_params["max_env_steps"]:
        ...
        # Update the record
        logger.update(
            reward, action=agent.action, position=next_state[2:4], crash=crash
        )
    ...
    # Save record
    logger.save_log(agent.replayer.loss_record)

    # Save model
    logger.save_model(
        policy_net, optimiser, agent.total_tstep, other_params["model_save_interval"]
    )

    # Plot progress"""
    logger.plot(
        e,
        agent.replayer.loss_record,
        other_params["sma_period_reward"],
        other_params["sma_period_loss"],
    )
